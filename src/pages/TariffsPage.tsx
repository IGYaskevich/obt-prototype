...
const tariffCards: {
   ...
   },
   {
      ...
      bullets: ['Отсрочка платежа', 'Лимиты постоплаты', 'Подсказки сроков оплаты', 'Сервисный сбор ~7%', 'Поддержка через чат/тикеты'],
   ...
   },
   {
      ...
      bullets: ['24/7 приоритетная поддержка', 'Любые обмены/возвраты', 'VIP-карточки и флоу', 'API-интеграции', 'Сервисный сбор ~4%'],
...
export default function TariffsPage() {
   ...
   return (
      <div className="space-y-6">
         <SectionHeader title="Тарифы и возможности" subtitle="Сравнение Free / Postpay / Flex и их поведения в продукте" />
         ...
         <div className="grid md:grid-cols-3 gap-4">
            <NoteCard
               ...
               text={['Флоу: вход → поиск → покупка.', 'Оплата только личной картой.', 'Подсветка политик без обязательных аппрувов.', 'Поддержки нет (только FAQ).']}
            />
            <NoteCard
               title="Как работает Postpay"
               text={['Появляется опция Postpay на оплате.', 'Показываем лимит и срок оплаты.', 'Если Out of Policy — требуется согласование.', 'Сервисный сбор ~7%.']}
            />
            <NoteCard title="Как работает Flex" text={['Включены обмены/возвраты.', 'VIP-поддержка 24/7.', 'Белые VIP-карточки в UI.', 'Возможны API-интеграции (demo-маркер).']} />
         </div>
      </div>
   )
}
