...
export default function CompanySignupPage() {
  ...
  const canGoStep1Next = useMemo(
    () => companyName.trim().length > 1 && companyBin.trim().length >= 6 && adminEmail.trim().length > 3,
      ...
      <div className="flex flex-col lg:flex-row gap-4">
        <div className="flex-1">
          ...
          <div className="mt-2 text-[11px] text-slate-500">
            Prototype note: no real payments are made, card data is not sent anywhere. The flow mirrors how real sign-up
            + card linking would look.
          </div>
        </div>
  ...
  return (
    <div className="grid md:grid-cols-2 gap-5 text-xs">
      ...
      <div className="space-y-3">
        ...
        <div className="text-[11px] text-slate-500">
          This user will be created as <strong>company admin</strong> with full access to settings, policies, employees
          and payment methods.
        </div>
      </div>
    </div>
  ...
  return (
    <div className="space-y-4 text-xs">
      ...
      <div className="border-t border-slate-100 pt-3 text-[11px] text-slate-500">
        {isFree ? (
          <>
            With <strong>Free</strong> you will go straight into the booking flow after sign-up: login → search → buy a
            ticket in 1–2 steps.
          </>
        ) : (
          <>
            For <strong>Postpay / Flex</strong> we also ask to link a corporate card on the next step. It will be used
            as a default payment method for your company.
          </>
        )}
      </div>
    </div>
  ...
  return (
    <div className="grid md:grid-cols-2 gap-5 text-xs">
      ...
      <div className="space-y-3 text-[11px] text-slate-600">
        ...
        <p>
          We will simulate a small verification charge (e.g. 100 ₸) to verify the card and then reverse it. The card
          will be stored as a <strong>corporate payment method</strong> for your company.
        </p>
        ...
        <label className="mt-2 flex items-start gap-2">
          ...
          />
          <span>
            I agree with terms of service and confirm that this card may be used as a corporate payment method for
            business travel.
          </span>
        </label>
      </div>
