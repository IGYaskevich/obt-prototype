...
export default function DocumentsPage() {
   ...
   const [statusFilter, setStatusFilter] = useState<'ALL' | 'WITH_DOCS' | 'WITHOUT_DOCS'>('ALL')
   ...
   const filteredRows = useMemo(() => {
      return rows.filter(r => {
         ...
         if (statusFilter === 'WITH_DOCS') {
            const anyDocReady = r.hasInvoice === 'READY' || r.hasAct === 'READY' || r.hasReceipt === 'READY'
            if (!anyDocReady) return false
         }

         if (statusFilter === 'WITHOUT_DOCS') {
            const allMissing = r.hasInvoice === 'NOT_AVAILABLE' && r.hasAct === 'NOT_AVAILABLE' && r.hasReceipt === 'NOT_AVAILABLE'
   ...
   const handleDownloadSingle = (row: DocumentRow, docType: 'invoice' | 'act' | 'receipt') => {
      const status = docType === 'invoice' ? row.hasInvoice : docType === 'act' ? row.hasAct : row.hasReceipt
   ...
   const handleDownloadAll = (row: DocumentRow) => {
      const anyReady = row.hasInvoice === 'READY' || row.hasAct === 'READY' || row.hasReceipt === 'READY'
   ...
   return (
      <div className="space-y-5">
         <SectionHeader title="Documents" subtitle="List of completed bookings with closing documents (invoice, act, receipt)." />
         ...
         <div className="card p-4 text-xs text-slate-600 flex flex-col md:flex-row md:items-center md:justify-between gap-3">
            <div className="flex items-start gap-2">
               ...
               <span>
                  Here you can see all bookings that produce closing documents. Payments from company balance, postpay or corporate card generate documents; personal card bookings
                  do not.
               </span>
            </div>
            {company && (
            ...
            <div className="grid gap-2 md:grid-cols-3 w-full md:w-auto">
               <div>
                  ...
                  <div className="relative mt-1">
                     ...
                     <input className="input pl-7 h-8 text-xs" placeholder="Type to search..." value={search} onChange={e => setSearch(e.target.value)} />
                  </div>
               </div>
               <div>
                  <label className="text-[11px] text-slate-500">Documents</label>
                  <select className="select mt-1 h-8 text-xs" value={statusFilter} onChange={e => setStatusFilter(e.target.value as 'ALL' | 'WITH_DOCS' | 'WITHOUT_DOCS')}>
                     <option value="ALL">All bookings</option>
                     <option value="WITH_DOCS">With documents</option>
                     <option value="WITHOUT_DOCS">Without documents</option>
                  </select>
               </div>
               <div className="hidden md:block text-[11px] text-slate-400">Use Reports page for detailed aggregation by period and payment methods.</div>
            </div>
         </div>
         ...
         <div className="card p-0 overflow-hidden">
            <table className="w-full text-sm border-collapse">
               ...
               <tbody>
                  {filteredRows.map(row => {
                     const anyReady = row.hasInvoice === 'READY' || row.hasAct === 'READY' || row.hasReceipt === 'READY'
                     return (
                        <tr key={row.id} className="border-t border-slate-100 hover:bg-slate-50/60 text-xs">
                           ...
                           <td className="px-3 py-2 text-xs">
                              {row.amount.toLocaleString('ru-RU')} <span className="text-slate-500">{row.currency}</span>
                           </td>
                           <td className="px-3 py-2 text-xs">
                              ...
                              {row.paymentMethod === 'PERSONAL_CARD' && <div className="text-[10px] text-slate-500">No closing documents for personal card payments.</div>}
                           </td>
                           <td className="px-3 py-2 text-xs">
                              <div className="flex flex-wrap gap-1">
                                 <DocBadge label="Invoice" status={row.hasInvoice} icon={<FileSpreadsheet size={11} />} onClick={() => handleDownloadSingle(row, 'invoice')} />
                                 <DocBadge label="Act" status={row.hasAct} icon={<FileText size={11} />} onClick={() => handleDownloadSingle(row, 'act')} />
                                 <DocBadge label="Receipt" status={row.hasReceipt} icon={<Receipt size={11} />} onClick={() => handleDownloadSingle(row, 'receipt')} />
                              </div>
                           </td>
            ...
            {filteredRows.length === 0 && <div className="p-4 text-xs text-slate-500 text-center">No bookings match current filters.</div>}
         </div>
      </div>
   )
}
...
function DocBadge({ label, status, icon, onClick }: { label: string; status: DocumentRowStatus; icon: React.ReactNode; onClick: () => void }) {
   let className = 'inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[11px] cursor-pointer transition'
   let text = label
   let disabled = false

   if (status === 'READY') {
      className += ' border border-emerald-200 text-emerald-700 bg-emerald-50 hover:bg-emerald-100'
   } else if (status === 'PENDING') {
      className += ' border border-amber-200 text-amber-700 bg-amber-50 hover:bg-amber-100'
      text = `${label} (pending)`
   } else if (status === 'NOT_AVAILABLE') {
      className += ' border border-slate-200 text-slate-400 bg-slate-50 cursor-not-allowed'
      text = `${label} (n/a)`
      disabled = true
   }
