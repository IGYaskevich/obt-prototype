  ...
  const filteredRows = useMemo(() => {
    return rows.filter(r => {
      ...
      if (statusFilter === 'WITHOUT_DOCS') {
        const allMissing =
          r.hasInvoice === 'NOT_AVAILABLE' && r.hasAct === 'NOT_AVAILABLE' && r.hasReceipt === 'NOT_AVAILABLE'
    ...
    <div className="space-y-5">
      ...
      <div className="card p-4 text-xs text-slate-600 flex flex-col md:flex-row md:items-center md:justify-between gap-3">
        <div className="flex items-start gap-2">
          ...
          <span>
            Here you can see all bookings that produce closing documents. Payments from company balance, postpay or
            corporate card generate documents; personal card bookings do not.
          </span>
        </div>
        {company && (
        ...
        <div className="grid gap-2 md:grid-cols-3 w-full md:w-auto">
          ...
          <div>
            ...
            <select
              ...
              value={statusFilter}
              onChange={e => setStatusFilter(e.target.value as 'ALL' | 'WITH_DOCS' | 'WITHOUT_DOCS')}
            >
      ...
      <div className="card p-0 overflow-hidden">
                ...
                <tr key={row.id} className="border-t border-slate-100 hover:bg-slate-50/60 text-xs">
                  ...
                  <td className="px-3 py-2 text-xs">
                    ...
                    {row.paymentMethod === 'PERSONAL_CARD' && (
                      <div className="text-[10px] text-slate-500">No closing documents for personal card payments.</div>
                    )}
                  </td>
                  <td className="px-3 py-2 text-xs">
        ...
        {filteredRows.length === 0 && (
          <div className="p-4 text-xs text-slate-500 text-center">No bookings match current filters.</div>
        )}
      </div>
    </div>
  )
