...
export default function EmployeeProfilePage() {
   ...
   const { getEmployeeById, updateEmployeeRole, addEmployeeDocument, removeEmployeeDocument, employeeHasValidDocs } = useStore()
      ...
      <div className="space-y-6">
         ...
         <div className="card p-4 flex flex-col md:flex-row md:items-center md:justify-between gap-4 text-sm">
            <div>
               ...
               <div className="text-xs text-slate-500 mt-1">
                  Role:{' '}
                  <select className="select h-7 text-xs" value={employee.role} onChange={e => updateEmployeeRole(employee.id, e.target.value as Role)}>
            ...
            <div className="text-xs flex flex-col gap-2 md:items-end">
               ...
               <div className="text-[11px] text-slate-500">
                  {canTravel ? 'Employee can be used as traveler in bookings.' : 'Employee cannot be used for booking until a valid passport or ID is added.'}
               </div>
            </div>
         </div>
         ...
         <div className="card p-4 space-y-3">
            <div className="flex items-center justify-between">
               ...
               <div className="text-[11px] text-slate-500">Add at least one valid passport or ID card to allow flight bookings.</div>
            </div>
               ...
               <table className="w-full text-xs border-collapse">
                  ...
                  <tbody>
                     {employee.documents.map(d => {
                        const statusColor = d.status === 'VALID' ? 'text-emerald-600' : d.status === 'EXPIRED' ? 'text-red-600' : 'text-slate-500'
                        return (
                           <tr key={d.type + d.number} className="border-t border-slate-100">
                              ...
                              <td className="px-2 py-1 text-right">
                                 <button className="btn-ghost text-xs text-red-600" onClick={() => removeEmployeeDocument(employee.id, d.number)}>
                                    <Trash2 size={12} />
                                    Remove
                                 </button>
                              </td>
            ...
            <div className="mt-3 border-t border-slate-100 pt-3">
               ...
               <div className="grid md:grid-cols-4 gap-3 text-xs">
                  <div>
                     ...
                     <select className="select mt-1 h-8 text-xs" value={docType} onChange={e => setDocType(e.target.value as EmployeeDocumentType)}>
                  ...
                  <div>
                     ...
                     <input className="input mt-1 h-8" value={docNumber} onChange={e => setDocNumber(e.target.value)} placeholder="Document number" />
                  </div>
                  <div>
                  ...
                  <div>
                     ...
                     <input className="input mt-1 h-8" value={docCountry} onChange={e => setDocCountry(e.target.value)} placeholder="KZ, TR, EU..." />
                  </div>
               </div>
