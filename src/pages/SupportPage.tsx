...
import { LifeBuoy, MessageCircle, Mail, PhoneCall, Clock, Zap, AlertTriangle, BookOpen, CheckCircle2, ExternalLink } from 'lucide-react'
...
export default function SupportPage() {
   ...
   return (
      <div className="space-y-6">
         <SectionHeader title="Support" subtitle="We help you resolve issues with bookings, payments, documents and policies." />
         ...
         <div className="card p-4 grid md:grid-cols-3 gap-4 text-sm">
            ...
            <div className="text-xs text-slate-500 flex items-start gap-2">
               ...
               <span>This is a demo. All actions here (chat, WhatsApp, ticket creation) are simulated with alerts, but they mimic real B2B OBT support flows.</span>
            </div>
         </div>
         ...
         <div className="card p-4 grid md:grid-cols-3 gap-4 text-sm">
               ...
               <div className="text-slate-600">
                  {isChatAvailable ? (
                     <>
                        ...
                        <div className="mt-2 flex flex-wrap gap-2">
                           <button
                              ...
                              onClick={() => alert('Simulated: open in-app chat widget. In real product this would open chat panel.')}
                           >
                     ...
                     <div className="text-[11px] text-slate-500">
                        Chat and WhatsApp support are available for Postpay and Flex tariffs. For Free tariff, please use email and FAQ below.
                     </div>
                  )}
               </div>
            </div>
            ...
            <div className="space-y-2 text-xs">
               ...
               <div className="text-slate-600">
                  ...
                  <div className="mt-2">
                     <span className="font-mono text-[12px] bg-slate-50 px-2 py-1 rounded border border-slate-200">support@demo-obt.com</span>
                  </div>
                  <div className="mt-1 text-[11px] text-slate-500">
                     Typical response: {company.tariff === 'FLEX' ? 'under 1 hour' : company.tariff === 'POSTPAY' ? '2–4 hours (business days)' : 'no guaranteed SLA'}
                  </div>
               </div>
            </div>
            ...
            <div className="space-y-2 text-xs">
               ...
               {isPhoneAvailable ? (
                  <div className="text-slate-600">
                     ...
                     <div className="mt-2 font-mono text-[12px] bg-slate-50 px-2 py-1 rounded border border-slate-200 inline-block">+7 (700) 123 45 67</div>
                  ...
                  <div className="text-[11px] text-slate-500">Priority phone line is available for Flex tariff. On Free/Postpay, main contact channels are email and chat.</div>
               )}
            </div>
         </div>
         ...
         <div className="card p-4 grid md:grid-cols-3 gap-4 text-sm">
            ...
            <div className="md:col-span-2 space-y-3 text-xs">
               ...
               <div className="flex justify-end">
                  <button className="btn-primary flex items-center gap-1 text-xs disabled:opacity-60" onClick={handleCreateTicket} disabled={!description.trim()}>
                     <LifeBuoy size={14} />
...
function SuggestedSteps({ issueType, severity, tariff }: { issueType: IssueType; severity: Severity; tariff: 'FREE' | 'POSTPAY' | 'FLEX' }) {
   const items: string[] = []

   // базовая логика по типу
   switch (issueType) {
      case 'BOOKING':
         items.push('Prepare booking details: route, dates, passenger names, PNR/booking ID.', 'Check if you see ticket number / e-ticket in Documents or Orders page.')
         break
      case 'PAYMENT':
         items.push('Check if payment transaction is visible in your bank / PSP.', 'Collect screenshots with payment status and any error messages.')
         break
      case 'DOCUMENTS':
         items.push(
         ...
         break
      case 'POLICY':
         items.push('Note which option is blocked: route, airline, time, price.', 'Compare with travel policy page — price limits and time windows may block this option.')
         break
      case 'TECH':
         items.push('Try to re-login and repeat the action in another browser.', 'Take screenshots of the error and copy the exact error message if available.')
         break
      case 'OTHER':
   ...
   if (severity === 'HIGH') {
      if (tariff === 'FLEX') {
         items.push('For HIGH urgency on Flex, please use phone or chat immediately — then send details by email or ticket so we can track the case.')
      } else if (tariff === 'POSTPAY') {
         items.push('For HIGH urgency on Postpay, create ticket here and use chat/WhatsApp if available. Mark booking as urgent in the subject.')
      } else {
         items.push('For HIGH urgency on Free, send detailed email and use all available channels (email + WhatsApp if listed by your manager).')
      }
