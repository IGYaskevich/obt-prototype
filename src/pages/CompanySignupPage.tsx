...
export default function CompanySignupPage() {
    ...
    const [companySize, setCompanySize] =
        useState<'SMALL' | 'MID' | 'ENTERPRISE'>('SMALL')
    ...
    /** валидация */
    const canGoStep1Next = useMemo(
        () =>
            companyName.trim().length > 1 &&
            companyBin.trim().length >= 6 &&
            adminEmail.trim().length > 3,
        [companyName, companyBin, adminEmail]
    )
            ...
            <div className="card p-4 text-xs space-y-3">
                ...
                <div className="space-y-2">
                    ...
                    <div>
                        ...
                        {!isFree && (
                            <div className="text-[11px] text-slate-500">
                                Expected monthly spend: ~{expectedMonthlySpend.toLocaleString()} ₸ ·{' '}
                                Due date: {preferredDueDays} days
                            </div>
                        )}
                    </div>
                    {!isFree && (
                ...
                <div className="pt-2 border-t border-slate-100 text-[11px] text-slate-500">
                    After sign-up you&apos;ll be redirected to{' '}
                    {isFree ? 'Search (first ticket fast flow).' : 'Dashboard to manage trips and company.'}
                </div>
            </div>
        </div>
    )
    ...
    return (
        <div className="space-y-6">
            <SectionHeader
                ...
                subtitle="Create a company admin account, choose tariff and (if needed) link a corporate card."
            />
            ...
            <div className="flex items-center gap-3 text-xs text-slate-600 mb-2">
                ...
                <ArrowRight size={14} className="text-slate-400" />
                <StepDot
                    active={step >= 3 || isFree}
                    label={isFree ? 'Confirm' : 'Payment & card'}
                    index={3}
                />
            </div>
                ...
                <div className="flex-1">
                    <div className="card p-5 space-y-5 text-sm">
                        ...
                        <div className="border-t border-slate-100 pt-4 flex items-center justify-between text-xs">
                            <button
                                className="btn-ghost flex items-center gap-1 text-xs text-slate-600 disabled:opacity-40"
                                onClick={handleBack}
                    ...
                    <div className="mt-2 text-[11px] text-slate-500">
                        Prototype note: no real payments are made, card data is not sent anywhere. The flow
                        mirrors how real sign-up + card linking would look.
                    </div>
                </div>
            ...
            <div className="space-y-3">
                ...
                <div>
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <input
                            className="input mt-1 text-xs flex-1"
                            value={adminEmail}
                ...
                <div className="text-[11px] text-slate-500">
                    This user will be created as <strong>company admin</strong> with full access to settings,
                    policies, employees and payment methods.
                </div>
            </div>
        </div>
    )
        ...
        <div className="space-y-4 text-xs">
            <div className="grid md:grid-cols-3 gap-3">
                <TariffCard
                    ...
                    bullets={[
                        'Simple booking flow',
                        'Documents for corporate card / balance',
                        'FAQ + email support',
                    ]}
                    active={tariff === 'FREE'}
                ...
                <TariffCard
                    ...
                    bullets={[
                        'Postpay & credit limit',
                        'Company balance & invoices',
                        'Chat support & SLA',
                    ]}
                ...
                <TariffCard
                    ...
                    bullets={[
                        'Flexible changes / refunds (where allowed)',
                        'Priority 24/7 care line',
                ...
                <div className="grid md:grid-cols-2 gap-4 mt-2">
                    <div>
                        <label className="text-[11px] text-slate-500">
                            Expected monthly travel spend (estimate)
                        </label>
                        <input
                            type="number"
                            className="input mt-1 text-xs"
                            value={expectedMonthlySpend}
                            onChange={e => setExpectedMonthlySpend(Number(e.target.value) || 0)}
                        />
                        <div className="text-[11px] text-slate-500 mt-1">
                            This helps us suggest a starting credit limit and configure reporting.
                        </div>
                    </div>
                    <div>
                        <label className="text-[11px] text-slate-500">
                            Preferred payment due date after trip (days)
                        </label>
                        <input
                            type="number"
                            className="input mt-1 text-xs"
                            value={preferredDueDays}
                            onChange={e => setPreferredDueDays(Number(e.target.value) || 0)}
                        />
                        <div className="text-[11px] text-slate-500 mt-1">
                            For Postpay / Flex we will show reminders “payment due in X days” in the UI.
                        </div>
                    </div>
                </div>
            )}
            ...
            <div className="border-t border-slate-100 pt-3 text-[11px] text-slate-500">
                {isFree ? (
                    <>
                        With <strong>Free</strong> you will go straight into the booking flow after sign-up:
                        login → search → buy a ticket in 1–2 steps.
                    </>
                ) : (
                    <>
                        For <strong>Postpay / Flex</strong> we also ask to link a corporate card on the next
                        step. It will be used as a default payment method for your company.
                    </>
                )}
            </div>
        </div>
...
function TariffCard({
    ...
    return (
        ...
        >
            ...
            <div className="mt-3 text-[11px] text-brand-700">
    ...
    return (
        <div className="grid md:grid-cols-2 gap-5 text-xs">
            ...
            <div className="space-y-3 text-[11px] text-slate-600">
                ...
                <p>
                    We will simulate a small verification charge (e.g. 100 ₸) to verify the card and then
                    reverse it. The card will be stored as a <strong>corporate payment method</strong> for
                    your company.
                </p>
                ...
                <label className="mt-2 flex items-start gap-2">
                    ...
                    />
                    <span>
            I agree with terms of service and confirm that this card may be used as a corporate
            payment method for business travel.
          </span>
                </label>
            </div>
        </div>
