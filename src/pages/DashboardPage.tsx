      ...
      <div className="card p-4 space-y-3">
        <div className="flex items-center justify-between">
          ...
          <div className="text-xs text-slate-500">
            {inboxTasks.length > 0 ? `${inboxTasks.length} item(s) requiring attention` : 'No urgent items (demo)'}
          </div>
        </div>
        ...
        {inboxTasks.length > 0 && (
          <div className="space-y-2">
            {inboxTasks.map(task => (
              <div key={task.id} className="flex items-center justify-between text-sm border-t border-slate-100 pt-2">
                <div>
                  <div
                    className={
                      task.severity === 'high'
      ...
      <div className="grid md:grid-cols-2 gap-4">
        ...
        <div className="card p-4 space-y-3">
          ...
          <div className="text-xs text-slate-500 mt-2">
            In this prototype, trips with status “Needs approval” are treated as potential policy violations
            (out-of-policy or over budget).
          </div>
        </div>
...
function KpiCard({
  ...
  return (
    <button
      onClick={onClick}
      className={`card p-4 text-left transition ${onClick ? 'hover:shadow-md cursor-pointer' : 'cursor-default'}`}
    >
      <div className="text-sm text-slate-500">{title}</div>
      <div
        className={`text-2xl font-semibold mt-1 ${
          accent === 'warn' ? 'text-amber-700' : accent === 'ok' ? 'text-emerald-700' : ''
        }`}
      >
        {value}
      </div>
...
function SmallKpi({ label, value, warn }: { label: string; value: number; warn?: boolean }) {
  return (
    <div
      className={`rounded-xl border border-slate-100 p-3 ${warn && value > 0 ? 'bg-amber-50/60 border-amber-200' : ''}`}
    >
